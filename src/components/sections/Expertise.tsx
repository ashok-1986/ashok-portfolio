...
function ExpertiseCard({ item, index }: { item: ExpertiseItem, index: number }) {
  ...
  return (
    <div
      ref={tiltRef as any}
      className={`card bg-[#1d0906] px-10 py-[44px] relative overflow-hidden transition-all cursor-default rev ${index === 0 ? 'd1' : index === 1 ? 'd2' : 'd3'}`}
    >
      ...
      <div className="card-label display-font text-[10px] font-bold tracking-[0.3em] text-[#FC4F2F] mb-6">
        {item.number}
      </div>
      <h3 className="card-title text-[22px] text-[#F0F3F5] mb-[18px] leading-[1.05]">
        {item.title}
      </h3>
  ...
  return (
    <section id="expertise" className="relative py-[120px] px-[60px] bg-[#1d0906]">
      <div className="exp-header flex justify-between items-end w-full max-w-[1200px] mb-[72px]">
        <h2 className="sec-title text-[clamp(52px,7.5vw,110px)] text-[#F0F3F5]">
          Expe<em className="text-[#FC4F2F] not-italic">rtise</em>
        </h2>
        <p className="exp-desc text-[14px] font-light leading-[1.75] text-[rgba(240,243,245,0.7)] max-w-[260px] text-right">
